    ...
    if (books.length === 0) {
      return (
        <div className="text-center py-20 bg-white rounded-xl border border-dashed border-gray-300">
          <p className="text-gray-500 text-lg">Aucun livre trouvé pour cette recherche.</p>
        </div>
      );
    }
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden flex flex-col hover:border-gray-300 transition-shadow hover:shadow-md">
      ...
      <div className="bg-gray-100 h-40 flex items-center justify-center relative">
        {coverImage
          ? <img src={coverImage} alt={title} className="h-32 w-auto rounded shadow-sm object-cover" />
          : <div className="w-20 h-28 bg-gray-200 rounded flex items-center justify-center text-gray-400 text-[10px] text-center px-1">No Cover</div>
        }
        <span className={`absolute top-2 right-2 text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider ${s.className}`}>
          {s.label}
        </span>
      </div>
      ...
      <div className="p-4 flex-1 flex flex-col gap-2">
        <p className="text-sm font-bold text-gray-900 line-clamp-2 leading-tight h-10">{title}</p>
        <p className="text-xs text-gray-500 truncate">
          {author}{publishedYear ? ` · ${publishedYear}` : ''}
        </p>

        {/* Genres */}
        <div className="flex flex-wrap gap-1 h-5 overflow-hidden">
          {genres.slice(0, 2).map(g => (
            <span key={g} className="text-[10px] px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 font-medium">
              {g}
            </span>
          ))}
        </div>
        ...
        <div className="mt-auto pt-2 border-t border-gray-100 flex justify-between text-[10px] text-gray-500 font-medium">
          <span>{availableCopies} / {totalCopies} dispos</span>
